...
def extractNDayFeature(path, date, n_day):
    book_map = {}
    add_map = {}
    del_map = {}
    download_map = {}
    ...
    for i in range(1,n_day+1):
        ...
        while line:
            ...
            read_date, dwellTime = time.split(' ')
            if eventId == '1':
...
def printDate(bookFeatures_arr,date,n_day):
    output = open('/Users/phj/Documents/Postgraduate/BookData/BooksPredict/OriginalData/featureEngineering/Day' + str(n_day) +'/' + date ,'w')
    for key in bookFeatures_arr:
